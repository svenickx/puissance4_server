...
namespace p4_server.Config {
    internal class CreateGame {
        public static Game? checkPlayersAvailable() {
            ...
            MySqlConnection connection = new MySqlConnection(MySqlUtils.GetDBstring());
            ...
            try {
                ...
                while (reader.Read()) {
                    Player player = new Player(reader["id"].ToString(), reader["player"].ToString());
                    players.Add(player);
                }
                if (players.Count > 1) {
        ...
        public static Game CreateNewGame(List<Player> players) {
            Game game = new Game(Guid.NewGuid().ToString(), players[0], players[1]);
